...
export default function Show({ page, recentLeads = [] }) {
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            <h2 className="font-display text-xl font-semibold text-gray-900 dark:text-white">{page.title}</h2>
            ...
            <div className="py-8">
                <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        <StatCard
                            label="Views"
                            value={(page.views_count ?? 0).toLocaleString()}
                        />
                        <StatCard
                        ...
                        <StatCard
                            label="Published"
                            value={page.published_at ? new Date(page.published_at).toLocaleDateString() : '—'}
                            sub={page.status !== 'published' ? 'Not published' : undefined}
                        />
                    </div>
                                    ...
                                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                        {recentLeads.map((lead, i) => (
                                            <tr key={lead.id ?? i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                                <td className="px-5 py-3 text-sm text-gray-900 dark:text-white">{lead.name ?? '—'}</td>
                                                <td className="px-5 py-3 text-sm text-gray-600 dark:text-gray-300">{lead.email ?? '—'}</td>
                                                <td className="px-5 py-3 text-sm text-gray-600 dark:text-gray-300">{lead.phone ?? '—'}</td>
                                                <td className="px-5 py-3 text-xs text-gray-400 dark:text-gray-500">
                                                    {lead.utm_source ?? 'direct'}
                                                </td>
                                                <td className="px-5 py-3 text-xs text-gray-400 dark:text-gray-500">
                                                    {lead.created_at_human ?? lead.created_at ?? '—'}
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
