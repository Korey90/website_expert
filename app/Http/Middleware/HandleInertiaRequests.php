...
class HandleInertiaRequests extends Middleware
{
    ...
    public function share(Request $request): array
    {
        ...
        $currency = $currencyResolver->resolve($request, $locale);
        ...
        return [
            ...
            'currency' => $currency,
            'available_currencies' => $currencyResolver->supportedCurrencies(),
            'currency_settings' => $currencyResolver->metadata($currency),
    ...
    private function resolveNavItems(): array
    {
        try {
            ...
            return NavItem::active()
                ->ordered()
                ->get()
                ->filter(fn (NavItem $item) => blank($item->section_key) || in_array($item->section_key, $activeSectionKeys)
                )
                ->map(fn (NavItem $item) => [
                    'href' => $item->href,
