                ...
                ->schema([
                    TextEntry::make('_stats')
                        ...
                        ->state(function ($record) {
                            ...
                            $currency = $record->currency ?? FilamentCurrency::default();
                            $money = app(CurrencySummaryFormatter::class);
                            $invoiceTotals = $money->sumByCurrency(
                                $record->invoices()->whereNotIn('status', ['draft', 'cancelled'])->get(['currency', 'total'])
                            );
                            $invoicedInProjectCurrency = (float) ($invoiceTotals[$currency] ?? 0);
                            $budgetPct = $budget > 0 ? min(round($invoicedInProjectCurrency / $budget * 100), 100) : 0;
                            ...
                            $card = fn (string $label, string $value, string $sub, string $color, int $pct = -1) => '<div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-top:3px solid '.$color.';border-radius:10px;padding:16px 20px;flex:1;min-width:0;">'
                            ...
                            return '<div style="display:flex;gap:14px;">'
                                ...
                                .$card('Budget Spent', $money->formatGrouped($invoiceTotals, fallbackCurrency: $currency), 'of '.FilamentCurrency::format($budget, $currency).' ('.$budgetPct.'%)', '#34d399', $budgetPct)
                    ...
                    ->schema([
                        TextEntry::make('_sidebar')
                            ...
                            ->state(function ($record) {
                                ...
                                $row = fn (string $icon, string $label, string $value) => '<div style="display:flex;align-items:flex-start;gap:10px;padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);">'
                                ...
                                $money = app(CurrencySummaryFormatter::class);
                                $invoicePaid = $money->sumByCurrency(
                                    $record->invoices()->where('status', 'paid')->get(['currency', 'total'])
                                );
                                $invoiceTotal = $money->sumByCurrency(
                                    $record->invoices()->whereNotIn('status', ['draft', 'cancelled'])->get(['currency', 'total'])
                                );
                                ...
                                $html = '<div>'
                                    ...
                                    .'<span style="font-size:13px;font-weight:700;color:#e2e8f0;">'.($record->budget ? FilamentCurrency::format($record->budget, $currency) : '—').'</span>'
                                    .'</div>'
                                    .'<div style="display:flex;justify-content:space-between;margin-bottom:6px;">'
                                    .'<span style="font-size:13px;color:#64748b;">Invoiced</span>'
                                    .'<span style="font-size:13px;font-weight:700;color:#e2e8f0;">'.$money->formatGrouped($invoiceTotal, fallbackCurrency: $currency).'</span>'
                                    .'</div>'
                                    .'<div style="display:flex;justify-content:space-between;">'
                                    .'<span style="font-size:13px;color:#64748b;">Paid</span>'
                                    .'<span style="font-size:13px;font-weight:700;color:#4ade80;">'.$money->formatGrouped($invoicePaid, fallbackCurrency: $currency).'</span>'
                                    .'</div>'
                                    .'</div>'
                                    .'</div>';
                        ...
                        ->state(function ($record) {
                            ...
                            foreach ($invoices as $inv) {
                                ...
                                $html .= '<tr style="border-bottom:1px solid rgba(255,255,255,0.05);">'
                                    ...
                                    .'<td style="padding:10px 12px;text-align:right;font-weight:700;color:#e2e8f0;">'.FilamentCurrency::format($inv->total, $inv->currency ?? $currency).'</td>'
        ...
        return $form->schema([
            Section::make('Project Details')
                ...
                ->schema([
                    ...
                    Forms\Components\TextInput::make('budget')->numeric()->prefix(fn () => FilamentCurrency::symbol()),
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('status')
                    ...
                    ->color(fn ($state) => match ($state) {
                        'draft' => 'gray', 'active' => 'success', 'on_hold' => 'warning', 'completed' => 'primary', 'cancelled' => 'danger', default => 'gray'
                    }),
                Tables\Columns\TextColumn::make('budget')->money(fn (Project $record) => FilamentCurrency::tableCurrency($record))->sortable(),
